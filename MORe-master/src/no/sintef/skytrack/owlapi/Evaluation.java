...
static Logger logger =  LogManager.getRootLogger();
	...
	static long reasonerTimeOut = 2*60*60*1000;
	...
	public static void main(String[] args) {
		...
		if (reasonersNameList.contains("MORe"))
			reasonerFactoryMap.put("MORe", new MOReReasonerFactory());
	...
	private static void writeInfencesToFile(String filename, OWLReasoner reasoner){
		...
		try {
			...
			manager.saveOntology(exportedOntology, IRI.create(file.toURI()));
	...
	public static void printOntologyStatistics(Map<String, String> ontologiesMap) {
		for (String name : ontologiesMap.keySet()) {
			...
			if (onto != null) {
				...
				logger.info("Classes:" + onto.getClassesInSignature(true).size());
				logger.info("Individuals: " + onto.getIndividualsInSignature(true).size());
	...
	public static double performConsistencyEvaluation(OWLOntology ontology, OWLReasonerFactory reasonerFactory, String name) throws Exception {
	    ...
	    timer.schedule(task, reasonerTimeOut);
